...
class ConfigAPI(object):
    ...
    def init(self):
        ...
        val = str(input())
        ConfigAPI.__get_token_by_link(self, val)
        ConfigAPI.__vk_api_on(self)
    ...
    def __vk_api_on(self):
        token = ConfigAPI.__token
        try:
            if(token != None):
                session = vk.Session(token)

                ConfigAPI.api = vk.API(session, v=5.103)
                user = ConfigAPI.api.users.get(user_ids=1) #не работает...
